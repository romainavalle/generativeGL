...
vec2 field( vec3 position )
{
	...
	nPos.y -= iGlobalTime * .05;

	vec4 quat = vec4( 0., 1., 0., -iGlobalTime * .1 );
...
void main() {
	...
	vec2  screenPos    = squareFrame( iResolution );
	float col = pow( max( 0., 1.- length( screenPos - vec2( 0.,.35 ) ) ), 1.5 );
	gl_FragColor = vec4( vec3( col ), 1. );


	float cameraAngle   = 0.2 * iGlobalTime;
	float cameraRadius  = 20.+sin( iGlobalTime*.1);
